...
public class CartController {
	 ...
	 * String email, @RequestParam("ps") String psd, Model model, HttpSession session) {
	 * logger.info("eStoreProduct:CartController:if user sign in");
	 * 
	 * // Retrieve the products ArrayList from the model custCredModel cust = cdao.getCustomer(email, psd); if (cust !=
	 * null) { flag = true;
	 * 
	 * try { cdao.updatelastlogin(cust.getCustId()); session.setAttribute("customer", cust); model.addAttribute("fl",
	 * flag);
	 * 
	 * } catch (Exception e) { System.out.println(e); } if (alist != null) { for (ProductStockPrice psp : alist) {
	 * cartimp.addToCart(psp.getProd_id(), cust.getCustId()); // cartimp.updateinsert(alist, cust.getCustId()); }
	 * List<ProductStockPrice> products = cartimp.getCartProds(cust.getCustId()); model.addAttribute("products",
